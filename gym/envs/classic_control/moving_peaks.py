...
class MovingPeaksEnv(gym.Env):
    ...
    def __init__(self):
        self.bias = 15  # bias added to reward based on previous action
    ...
    def _step(self, action):
        assert action in self.action_space, "%r (%s) %s invalid"%(action, type(action), self.action_space)
        state = self.state
